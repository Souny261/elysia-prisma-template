...
export const response = {
    ...
    SuccessResponseMessage: (ctx: Context, message: string): ApiResponse<null> => {
        ctx.set.status = 200;
        return {
            status: true,
            error: message
    ...
    ErrorResponseMessage: (message: string): ApiResponse<null> => {
        return {
            status: false,
            error: message
        };
    ...
    ErrorResponse: (ctx: Context, error: string): ApiResponse<any> => {
        ctx.set.status = 400;
        return {
            status: false,
            error: error
    ...
    ErrorPrismaResponse: (set: any, error: any) => {
    ...
    PaginatedSuccessResponse: <T>(ctx: any, data: T[], pagination: PaginatedResponse<T>['pagination']): PaginatedResponse<T> => {
        ctx.set.status = 200;
        return {
            status: 'success',
            data: data,
...
function getPrismaErrorMessage(error: any) {
    ...
    return {
        message: `${message} Details: ${error.meta ? JSON.stringify(error.meta.target) : 'No additional details available.'}`,
        status: status,
    };
}